...
# saving path
paras.save_dir = RES_ROOT/f"simu_setting4g_{cs[0]*1000:.0f}"
...
def _run_fn(seed, lam, N, paras, is_save=False, verbose=False):
    ...
    def _inner_fn(cur_data, _paras, verbose=False):
        ...
        return _test_fn(_paras.Cmats[0], res), _test_fn1(_paras.Cmats[0], res)
...
with Parallel(n_jobs=35) as parallel:
    ress = parallel(delayed(_run_fn)(seed, lam=lam, N=N, paras=paras, is_save=True, verbose=False) 
                    for seed, lam, N 
