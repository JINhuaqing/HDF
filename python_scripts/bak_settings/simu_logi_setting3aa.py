...
# lambda: penalty term
# N: num of basis
paras.can_lams = [0.001, 0.05, 0.1, 0.13, 0.15, 0.18, 0.2, 0.25, 0.3, 0.35, 2.0, 4.0, 16.0]
paras.can_Ns = [5, 7]
...
paras.save_dir = RES_ROOT/f"simu_logi_setting3aa_{cs[0]*1000:.0f}"
...
can_Ns = [4, 5, 6, 7, 8, 10, 12]
all_coms = itertools.product(range(0, paras.num_rep_CV), paras.can_lams, can_Ns)
def _is_success(res):
    return res.success
...
with Parallel(n_jobs=30) as parallel:
    all_cv_errs_list = parallel(delayed(_run_fn)(cur_seed, cur_N, cur_lam, c1=cs[0])  
                                 for cur_seed, cur_lam, cur_N
                                 in tqdm(all_coms, total=paras.num_rep_CV*len(can_Ns)*len(paras.can_lams), 
                                                      desc=f"c1: {cs[0]}"))
all_cv_errs = {res[0]:res[1] for res in all_cv_errs_list};
save_pkl(paras.save_dir/f"all-valsel-metrics_c1_{cs[0]*1000:.0f}.pkl", all_cv_errs, is_force=1)
